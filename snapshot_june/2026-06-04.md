...
### ✅ Hermes
Hermes AI agent VM — Ubuntu 26.04 LTS, 4 GB RAM, IP `192.168.100.101`, exposed externally as `hermes.joeyme.eu`.  

- Runs the local tooling for Hermes AI Agent.
- Hermes Agent is powered by MiniMax-M3.
- **Ansible control node:** All playbooks live in `~/ansible` and target the rest of the homelab from here.  
...
A second Synology DSM instance, **virtualized** as a Proxmox VM (`vmid 102`, 4 GB RAM) rather than running on bare metal.   
...
Sysprepped / cloud-init-ready base images on PVE1.  
Ready to clone-and-deploy when a new VM is needed.
...
*Compose configs for both Docker hosts are mirrored under [`docker/`](./docker/) — 16 audited files. See the [README](./docker/README.md) for the audit manifest and excluded items.*
