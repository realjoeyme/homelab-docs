...
#### ⚙️ Node Optimizations & Monitoring

* **CPU Power Management:** To keep power draw and heat down on the i5-12500T, the CPU scaling governor is locked to `powersave`.
  ```bash
  [root@pve1 ~]$ cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor
  powersave

* **ProxMenux Setup:** Configured via the interactive `menu` utility, which provides an advanced web dashboard monitoring application for Proxmox VE. It tracks real-time node vitals, hardware temperatures, and active VM states at a glance.
![Dashboard](proxmenux.png)
* **Glances (Web Server Mode):** Installed Glances locally on `pve1` running in Web Server mode. This provides a fast terminal-style system overview and directly pipes raw host performance metrics straight into my main Homepage dashboard.
![Dashboard](glances.png)
---
...
| <img src="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/synology-dsm.png" width="20" height="20" alt="Synology DSM"> | `ds2` | 102 | DSM (virtualized) 4.4.302+ | 192.168.100.26 | 4GB | Virtual Synology |
...
Beyond the agent itself, Hermes runs a handful of scheduled automations: a daily **morning infrastructure & AI brief** (8:30 AM, pushed to Telegram), daily **Proxmox health check** at 7 AM, daily **TV procurement heartbeat** at noon, weekly **homelab infrastructure overview** regen (Mondays 7 AM), and a weekly **Pulse DB vacuum** (Sundays 5 AM). Most are script-only (no LLM in the loop) — keeps the dependency surface tight.
...
Domain controller, DHCP server, authoritative DNS for `local.home`, and Stratum-2 NTP source (syncs from `time.google.com`). **DHCP scope:** `192.168.100.2–.253`, hands both Pi-holes (`.59`, `.69`) as DNS. Runs the `local.home` AD domain and GPOs for a handful of older W11 PCs around the house.
